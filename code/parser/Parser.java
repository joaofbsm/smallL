...
public class Parser {
    private Lexer lex;  // analisador l\’exico para este analisador sint\’atico
    private Token look;  // lookahead token
    Env top = null;  // tabela de s\’imbolos corrente ou do topo
    int used = 0;  // mem\’oria usada para declara\c{c}\~oes
    ...
    Stmt stmt() throws IOException {
        Expr x;  Stmt s, s1, s2;
        Stmt savedStmt;         // guarda o la\c{c}o
        switch( look.tag ) {
            case ;:
                move();
                return Stmt.null;
    ...
    Stmt assign() throws IOException {
        ...
        if( id == null ) error(t.toString() + " undeclared"); if(look.tag===){  //S->id=E;
            move();  stmt = new Set(id, bool());
        }
        else{ //S->L=E; Access x = offset(id);
